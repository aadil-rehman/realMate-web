...
const EditBlog = () => {
	...
	const hadleBlogSave = async () => {
		try {
			const updatedData = {};
			if (blogTitle !== blog.blogTitle) updatedData.blogTitle = blogTitle;
			if (blogDescription !== blog.blogDescription)
				updatedData.blogDescription = blogDescription;
			if (
				blogImage &&
				JSON.stringify(blogImage) !== JSON.stringify(blog.blogImage)
			) {
				updatedData.blogImage = blogImage;
			}

			if (Object.keys(updatedData).length === 0) {
				navigate(`/app/blog/view/${blogId}`);
				return;
			}
